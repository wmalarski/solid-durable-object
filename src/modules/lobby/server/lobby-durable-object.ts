...
export class LobbyDurableObject extends DurableObject<Env> {
  ...
  joinLobby(userIp: string) {
    ...
    if (!lobby || lobby.userIps.length >= MAX_LOBBY_SIZE) {
      ...
      this.lobbies.set(newLobbyId, { id: newLobbyId, userIps: [userIp] });
      this.readyLobbies.push(newLobbyId);
      this.userLobbies.set(userIp, newLobbyId);

      return newLobbyId;
    }

    lobby.userIps.push(userIp);
    this.userLobbies.set(userIp, lobby.id);
