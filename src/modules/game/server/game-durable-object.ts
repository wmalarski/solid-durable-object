...
export class ChatRoom {
  constructor(state, env) {
    ...
    this.state.getWebSockets().forEach((webSocket) => {
      ...
      this.sessions.set(webSocket, { ...meta, blockedMessages, limiter });
    });
  ...
  async handleSession(webSocket, ip) {
    ...
    const session = { blockedMessages: [], limiter, limiterId };
    ...
    // client.
    const storage = await this.storage.list({ limit: 100, reverse: true });
  ...
  async webSocketMessage(webSocket, msg) {
    try {
      ...
      data = { message: "" + data.message, name: session.name };
